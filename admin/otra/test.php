...
<?php
	...
	function subir_imagen($file){
		...
		if($file && $file["name"]!=""){
			...
			$direccion_web = "http://www.movivet.tk/admin/img/";
			$nombre_carpeta = "img/";
	...
	if(isset($_POST["submit"])){
		...
		$nombreImagenES = mysqli_real_escape_string($conexion,$_POST["nombreImagenES"]);
		...
		$URLimagen = subir_imagen($_FILES["archivo_a_subir"]);
		...
		if($URLimagen=="error"){
			...
			$sql = "insert into imagenes(nombreImagenES, altES, URLimagen) values ('$nombreImagenES','$altES','$URLimagen')";
...
<body>
	<form action="test.php" method="post" enctype="multipart/form-data"> <!-- IMPORTANTISIMO AGREGAR EL ENCTYPE AL FORM, SINO NO FUNCIONA EL UPLOAD -->
	    ...
	    <p>
	    	...
	    	<input type="text" name="nombreImagenES" placeholder="Escribir nombre">
	    </p>
	    <p>
	    	<label>Descripcion</label>
	    	<input type="text" name="altES" placeholder="Escribir descripcion">
	    </p>
